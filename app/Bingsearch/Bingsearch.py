...
import uuid
import markdown
from ..config import bing_client, cohere_async_clients  # Import the settings
...
async def call_cohere_chat(
    messages: list[dict], model: str = "command-r-08-2024", temperature: float = 0.8
) -> str:
    """
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail="Error reading response from Cohere."
        )
...
USE NUMBERED INDENTED BULLETPOINTS AND LINEBREAKS AND INDENTS, HEADINGS AND PARAGRAPHS WITH EMPTY LINES IN BETWEEN. ALWAYS RETURN SOURCES WITH LINKED URLS.
 ...
 """
        ...
        raw_llm_response = await call_cohere_chat(
            messages_for_cohere, model="command-r-08-2024"
        )
