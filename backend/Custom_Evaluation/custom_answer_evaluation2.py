...
from backend.Custom_Evaluation.questionnaire_party_answers import questionnaire_party_answers, parties_list
from backend.clients import AzureOpenAIClientManager
from backend.Custom_Evaluation.eval_prompts import EVALUATION_PROMPT, GENERATE_LOOKUP_PROMPT, EVALUATION_PROMPT2




# ----------------------    UNTIL HERE IT WAS ONLY EXPLNTION OF THE CODE ----------------------
...
async def get_custom_answers_evaluation( 
    ...
    for  (answer) in enumerate( custom_answers):
        if answer.custom_answer == "":
            continue
        
        
        party_to_response = questionnaire_party_answers[answer.question_id]
        if len(parties_list) < 1:
            parties_list = [k for k, v in party_to_response.items()]
        parties = parties_list 
        
        # TODO: Split parties into main and smaller
        main_parties = ["CDU/CSU", "GRUENE", "SPD", "AfD", "FDP", "BSW"] 
        smaller_parties = ["DIE LINKE", "MLPD", "BÜNDNIS DEUTSCHLAND", "FREIE WÄHLER", "Volt"]
